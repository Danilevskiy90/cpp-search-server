...
vector<pair<int, int>> FindAllDocuments(const map<string, set<int>>& word_to_documents, const set<string>& stop_words, const string& query) 
{
    ...
    for (auto [document_id, relevance] : document_to_relevance) 
    {
        pair<int, int> s;
        s.first=relevance;
        s.second=document_id;
        found_documents.push_back(s);
    }
    return found_documents;
}

vector<pair<int, int>> FindTopDocuments(vector<pair<int, int>> found_documents)
{
...
int main() 
{
    ...
    vector<pair<int, int>> found_documents=FindAllDocuments(word_to_documents, stop_words, query);
    int i=0;
    for (auto [relevance,document_id] : FindTopDocuments(found_documents)) 
    {
        if (i==MAX_RESULT_DOCUMENT_COUNT)
        {
            break;
        }
